...
class DuckDuckGoHandler:
    def __init__(self, query: str):
        ...
        self.results_title = self.results_quick_links.find_all(
            "h2", attrs={"class": "result__title"}
        )

        self.results_info = self.results_quick_links.find_all(
        ...
        self.array_of_titles = (
            ((str(self.raw_html_title).replace("[", "")).replace("]", "")).strip()
        ).split(",")
        ...
        for iteration, quick_snippet in enumerate(self.results_info):
            self.results_info_snippet.append(
                BeautifulSoup(str(quick_snippet), "html.parser").find().text
            )
        ...
        for title in self.titles:
            for snippet in self.snippets:
                for link in self.links:
                    self.objects_list_duck.append(
                        DuckDuckGoResult(title, snippet, link)
                    )
